<?php
/*****************************************************************************************
	檔案： {phpok}/admin/task_control.php
	備註： 計劃工作管理員
	版本： 4.x
	網站： www.phpok.com
	作者： qinggan <qinggan@188.com>
	時間： 2015年10月21日 09時48分
*****************************************************************************************/
	...
	public function save_f()
	{
		...
		if($main['day'] && is_numeric($main['day']) && $main['month'] && is_numeric($main['month'])){
			if(in_array($main['month'],$day30) && $main['day'] == '31'){
				$this->json(P_Lang('當前月份不支援31日'));
			}
			if($main['month'] == '02' && $main['day'] > 29){
				$this->json(P_Lang('二月份不支援30日及31日'));
			}
			if($main['day'] == 29 && $main['year'] && is_numeric($main['year'])){
				if($main['year']%4){
					$this->json(P_Lang('當前月份不支援29日'));
				}
			}
		}
