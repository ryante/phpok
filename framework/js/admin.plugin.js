...
;(function($){
	$.admin_plugin = {
		icon:function(id,vid,title)
		{
			...
			$.dialog.open(url,{
				...
				'ok':function(){
					...
					iframe.save();
					return false;
				},'okVal':p_lang('儲存'),'cancel':true
			});
		},
		create:function()
		{
			$.dialog({
				'title':p_lang('建立一個新的外掛'),
				'width':'880px',
				'height':'280px',
				'lock':true,
				...
				'ok':function(){
					...
					$.phpok.json(url,function(rs){
						if(rs.status){
							$.dialog.alert(p_lang('外掛建立成功，請根據實際情況編寫外掛擴充套件'),function(){
								$.phpok.reload();
							},'succeed');
							return true;
						}
		...
		{
			$.dialog.confirm(p_lang('確定要解除安裝外掛 {title} 嗎？<br>解除安裝後相應的功能都不能使用','<span class="red">'+title+'</span>'),function(){
				var url = get_url('plugin','uninstall','id='+$.str.encode(id));
				$.phpok.json(url,function(rs){
					if(rs.status){
						$.dialog.tips(p_lang('解除安裝成功…'));
						$.phpok.reload();
						return true;
					}
					$.dialog.alert(rs.info);
		...
		config:function(id,title){
			...
			$.dialog.open(url,{
				'title':p_lang('設定外掛{title}相關引數',' <span class="red">'+title+'</span> '),
				'lock':true,
				'width':'790px',
				'height':'500px',
				'ok':function(){
		...
		{
			$.win(p_lang('外掛管理')+"_#"+index,get_url('plugin','setting','id='+id));
		},
		tozip:function(id)
		{
		...
		icon_del:function(id,vid,title)
		{
			...
			$.dialog.confirm(p_lang('確定要刪除外掛快捷操作項：{title}嗎？','<span class="red">'+title+'</span> '),function(){
		...
		{
			$.dialog.prompt(p_lang('設定要排序的數值，越小越往前靠，最小值為0'),function(val){
				if(!val){
					$.dialog.alert('排序值不能為空');
					return false;
				}
				if(val == old){
					$.dialog.alert('排序值不能和舊的一樣');
				}
