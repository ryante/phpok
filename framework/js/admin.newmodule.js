...
;(function($){
	$.admin_module = {
		...
		**/
		field_del:function(id,title)
		{
			...
			$.dialog.confirm(tip,function(){
				var url = get_url("newmodule","field_delete") + "&id="+id;
		...
		**/
		...
		{
			var url = get_url("newmodule","field_edit") + "&id="+id;
			$.dialog.open(url,{
		...
		**/
		...
		{
			var url = get_url("newmodule","field_add",'id='+id+'&fid='+fid);
			$.phpok.json(url,function(rs){
				if(rs.status){
					$.phpok.reload();
					return false;
		...
		**/
		field_addok:function(mid)
		{
			...
			$("#form_save").ajaxSubmit({
				'url':get_url('newmodule','field_addok','mid='+mid),
				'type':'post',
				'dataType':'json',
				'success':function(rs){
					if(rs.status){
		...
		**/
		...
		{
			$.dialog.open(get_url("newmodule","field_create","mid="+id),{
				'title':p_lang('添加字段'),
				'lock':true,
				'width':'650px',
				'height':'70%',
		...
		**/
		create:function()
		{
			$.dialog.open(get_url('newmodule','set'),{
				'title':p_lang('模块添加'),
				'lock':true,
				'width':'650px',
				'height':'400px',
		...
		**/
		input:function()
		{
			$.dialog.open(get_url('newmodule','import'),{
				'title':p_lang('模块导入'),
				'lock':true,
				'width':'500px',
				'height':'150px',
		...
		**/
		edit:function(id)
		{
			$.dialog.open(get_url('newmodule','set','id='+id),{
		...
		**/
		del:function(id,title)
		{
			$.dialog.confirm(p_lang('确定要删除模块：{title}？<br/>如果模块中有内容，也会相应的被删除，请慎用','<span style="color:red;font-weight:bold;">'+title+'</span>'),function(){
				var url = get_url("newmodule","delete")+"&id="+id;
				$.phpok.json(url,function(rs){
					if(rs.status){
						$.phpok.reload();
						return false;
		...
		**/
		export:function(id)
		{
			var url = get_url('newmodule','export','id='+id);
			$.phpok.go(url);
		},
		...
		**/
		...
		{
			$.dialog.open(get_url("newmodule","layout","id="+id),{
		...
		layout_save:function()
		{
			...
			$("#module_layout_save").ajaxSubmit({
				'url':get_url('newmodule','layout_save'),
				'type':'post',
				'dataType':'json',
				'success':function(rs){
					if(rs.status){
		...
		**/
		copy:function(id,title)
		{
			$.dialog.prompt(p_lang('请设置新模块的名称：'),function(val){
				...
				var url = get_url("newmodule","copy","id="+id+"&title="+$.str.encode(val));
				$.phpok.json(url,function(rs){
					if(rs.status){
						$.phpok.reload();
						return true;
		...
		**/
		status:function(id)
		{
			$.phpok.json(get_url("newmodule","status","id="+id),function(rs){
				if(rs.status){
					if(!rs.info){
						rs.info = '0';
					}
		...
		**/
		taxis:function(id,taxis)
		{
			$.dialog.prompt(p_lang('请填写新的排序：'),function(val){
				if(val != taxis){
					$.phpok.json(get_url('newmodule','taxis','taxis='+val+"&id="+id),function(rs){
						if(rs.status){
							$.phpok.reload();
							return true;
						}
